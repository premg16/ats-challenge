...
describe('calculateMatchScores', () => {
  ...
  const mockCV = {
    ...
    experience: [
      { company: "Tech Co", role: "Developer", years: 3 }
    ],
    education: [
  ...
  test('should call OpenAI with correct parameters when provider is openai', async () => {
    ...
    expect(PromptTemplate.fromTemplate).toHaveBeenCalledWith(PROFILE_MATCHING_PROMPT);
  ...
  test('should throw error for invalid provider', async () => {
    ...
    await expect(calculateMatchScores(mockCV, mockJob, 'invalid')).rejects.toThrow('Invalid provider');
  ...
  test('should format candidate profile correctly', async () => {
    ...
    const formatMock = promptTemplateMock.mock.results[1].value.format as jest.Mock;
    const formatCall = formatMock.mock.calls[0][0];
