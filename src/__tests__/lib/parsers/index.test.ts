...
describe("CV Parser", () => {
    ...
    it("should throw an error for unsupported file types", async () => {
        ...
        await expect(parseCV(file)).rejects.toThrow(
            "CV parsing error: Unsupported file type",
        );
    });

    it("should propagate errors from individual parsers", async () => {
        // Setup mock parser to throw an error
        (parsePDF as jest.Mock).mockRejectedValue(
        ...
        // Expect the function to throw
        await expect(parseCV(file)).rejects.toThrow(
            "CV parsing error: PDF parsing failed",
        );
    });
