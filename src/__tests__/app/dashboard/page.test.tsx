...
describe("Dashboard", () => {
    ...
    beforeEach(() => {
        ...
        (CVUpload as jest.Mock).mockImplementation(
            ({ onUpload, files, setFiles }) => (
                <div data-testid="cv-upload">
                    ...
                    <button
                        data-testid="mock-upload-error-button"
                        onClick={() => onUpload([{ error: "Test error" }])}
                    >
                        Mock Upload Error
                    </button>
                </div>
        ...
        (ResultsTable as jest.Mock).mockReturnValue(
            <div data-testid="results-table">Results Table</div>,
        );
    });
            ...
            results: [
                {
                    candidate: [
                        {
                            candidateDetails: {
                                personalDetails: { name: "Test Candidate" },
                            },
                        },
                    ],
                },
    ...
    test("allows removing files from the list", async () => {
        ...
        const removeFileFunction = (file: File) =>
            mockFiles.filter((f) => f !== file);
        ...
        const filterFunction = (files: File[]) =>
            files.filter((f) => f !== mockFiles[0]);
    ...
    test("filters out error results when processing uploads", async () => {
        ...
        const mixedResults = [
            {
                candidate: [
                    {
                        candidateDetails: {
                            personalDetails: { name: "Valid Candidate" },
                        },
                    },
                ],
            },
            { error: "Error 1" },
            {
                candidate: [
                    {
                        candidateDetails: {
                            personalDetails: { name: "Another Valid" },
                        },
                    },
                ],
            },
        ...
        (CVUpload as jest.Mock).mockImplementation(({ onUpload }) => (
            <div data-testid="cv-upload">
                <button
                    data-testid="mixed-upload-button"
                    onClick={() => onUpload(mixedResults)}
                >
