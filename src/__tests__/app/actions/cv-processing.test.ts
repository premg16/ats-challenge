...
describe('processCVs', () => {
  ...
  const mockAnalysisResult = {
    ...
    keyMatchingHighlights: ['JavaScript', 'React', 'Communication'] as [string, string, string],
  ...
  it('should process CVs successfully with OpenAI provider', async () => {
    ...
    (calculateMatchScores as jest.Mock).mockResolvedValue(mockAnalysisResult);
    ...
    expect(calculateMatchScores).toHaveBeenCalledWith(mockParsedCV, jobRolesData.jobRoles[0], 'openai');
    expect(calculateMatchScores).toHaveBeenCalledWith(mockParsedCV, jobRolesData.jobRoles[1], 'openai');
    ...
    expect(results[0].candidate[0].jobMatches[0].jobTitle).toBe('Software Engineer');
    expect(results[0].candidate[0].jobMatches[0].analysis).toEqual(mockAnalysisResult);
    expect(results[0].candidate[0].jobMatches[1].jobTitle).toBe('Product Manager');
    expect(results[0].candidate[0].jobMatches[1].analysis).toEqual(mockAnalysisResult);
  });
  ...
  it('should process CVs successfully with Gemini provider', async () => {
    ...
    expect(calculateMatchScores).toHaveBeenCalledTimes(2); // Once for each job role
    expect(calculateMatchScores).toHaveBeenCalledWith(mockParsedCV, jobRolesData.jobRoles[0], 'gemini');
    expect(calculateMatchScores).toHaveBeenCalledWith(mockParsedCV, jobRolesData.jobRoles[1], 'gemini');
  ...
  it('should handle CV parsing errors', async () => {
    // Setup mocks
    const mockFile = createMockFile('resume.pdf', 'application/pdf');
    (parseCV as jest.Mock).mockRejectedValue(new Error('PDF parsing failed'));
    ...
    expect(results[0].error).toBe('Failed to process CV resume.pdf: PDF parsing failed');
  });
  ...
  it('should handle extraction errors', async () => {
    ...
    const mockFile = createMockFile('resume.pdf', 'application/pdf');
    (parseCV as jest.Mock).mockResolvedValue(mockCVText);
    (extractDetails as jest.Mock).mockRejectedValue(new Error('Extraction failed'));
    ...
    expect(results).toHaveLength(1);
    expect(results[0].candidate).toHaveLength(0);
    expect(results[0].error).toBe('Failed to process CV resume.pdf: Extraction failed');
  });
  ...
  it('should handle match scoring errors', async () => {
    ...
    (parseCV as jest.Mock).mockResolvedValue(mockCVText);
    (extractDetails as jest.Mock).mockResolvedValue(mockParsedCV);
    (calculateMatchScores as jest.Mock).mockRejectedValue(new Error('Matching failed'));
    ...
    expect(results).toHaveLength(1);
    expect(results[0].candidate).toHaveLength(0);
    expect(results[0].error).toBe('Failed to process CV resume.pdf: Matching failed');
  });
  ...
  it('should handle mixed success and failure when processing multiple CVs', async () => {
    ...
    expect(successResult?.candidate[0].candidateDetails).toEqual(mockParsedCV);
    
    // Verify error result
    expect(errorResult).toBeDefined();
    expect(errorResult?.candidate).toHaveLength(0);
    expect(errorResult?.error).toBe('Failed to process CV resume2.pdf: PDF parsing failed');
  });
