...
jest.mock('@/components/ui/button', () => ({
  Button: jest.fn().mockImplementation(({ children, size, variant, className, ...props }) => (
    <button 
...
describe('Page', () => {
  it('renders the hero section correctly', () => {
    ...
    expect(screen.getByText((content) => content.trim() === 'Hiring')).toBeInTheDocument();
    expect(screen.getByText('AI-powered candidate ranking system that revolutionizes how you hire for blue-collar & sales roles.')).toBeInTheDocument();
    
    // Check hero section buttons
    const getStartedButton = screen.getByText('Get Started').closest('[data-testid="mock-button"]');
    expect(getStartedButton).toBeInTheDocument();
    expect(getStartedButton).toHaveAttribute('data-size', 'lg');
    
    const watchDemoButton = screen.getByText('Watch Demo').closest('[data-testid="mock-button"]');
  ...
  it('renders the features section correctly', () => {
    ...
    expect(screen.getByText('Everything you need to streamline your recruitment process')).toBeInTheDocument();
    ...
    expect(screen.getByText('Bulk upload CVs in any format. Our AI handles the rest.')).toBeInTheDocument();
    expect(screen.getByText('Advanced algorithms extract and analyze candidate information with high precision.')).toBeInTheDocument();
    expect(screen.getByText('Get instant candidate rankings with detailed match scores.')).toBeInTheDocument();
  ...
  it('renders the CTA section correctly', () => {
    ...
    expect(screen.getByText('Join hundreds of companies already using Recruit Raid to find their perfect candidates.')).toBeInTheDocument();
