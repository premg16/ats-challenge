...
jest.mock('@/components/ui/sonner', () => ({
  Toaster: jest.fn().mockImplementation(({ richColors }) => (
    <div data-testid="mock-toaster">Mocked Toaster</div>
  ))
}));

jest.mock('@/components/theme-provider', () => ({
  ThemeProvider: jest.fn().mockImplementation(({ children, attribute, defaultTheme, enableSystem }) => (
    <div 
...
jest.mock('@/components/ui/header', () => ({
  __esModule: true,
  default: jest.fn().mockImplementation(() => <div data-testid="mock-header">Mocked Header</div>)
}));
...
describe('RootLayout', () => {
  ...
  it('renders children within ThemeProvider', () => {
    ...
    expect(getByTestId('test-children')).toHaveTextContent('Test Children Content');
  });
