...
const Tabs = ({ children, defaultValue }: any) => <div data-testid="tabs-root">{children}</div>;
const TabsList = ({ children, className }: any) => <div data-testid="tabs-list" className={className}>{children}</div>;
const TabsTrigger = ({ children, value, className }: any) => <button data-testid="tabs-trigger" className={className}>{children}</button>;
const TabsContent = ({ children, value, className }: any) => <div data-testid="tabs-content" className={className}>{children}</div>;
...
jest.mock('@radix-ui/react-tabs', () => ({
  ...
  List: forwardRef(({ children, ...props }: any, ref: any) => (
    <div ref={ref} data-testid="tabs-list" {...props}>{children}</div>
  )),
  Trigger: forwardRef(({ children, ...props }: any, ref: any) => (
    <button ref={ref} data-testid="tabs-trigger" {...props}>{children}</button>
  )),
  Content: forwardRef(({ children, ...props }: any, ref: any) => (
    <div ref={ref} data-testid="tabs-content" {...props}>{children}</div>
  )),
}));

// Mock the actual tabs component
jest.mock('@/components/ui/tabs', () => ({
  Tabs: ({ children, ...props }: any) => <div data-testid="tabs-root" {...props}>{children}</div>,
...
describe('Tabs components', () => {
  ...
  describe('TabsList', () => {
    ...
    it('applies custom classes', () => {
      ...
      const tabsList = screen.getByTestId('tabs-list');
      expect(tabsList).toHaveAttribute('class', expect.stringContaining('custom-list-class'));
    });
