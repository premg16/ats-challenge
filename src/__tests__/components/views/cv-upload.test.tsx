...
describe('CVUpload', () => {
  ...
  it('validates file types and rejects invalid files', async () => {
    ...
    expect(toast.error).toHaveBeenCalledWith('test.jpg is not a supported file type');
    expect(mockSetFiles).not.toHaveBeenCalled();
  });

  it('handles upload button click', async () => {
    const mockFilesWithContent = [new File(['test content'], 'test.pdf', { type: 'application/pdf' })];
    ...
    await waitFor(() => {
      expect(processCVs).toHaveBeenCalledWith(mockFilesWithContent, 'openai');
      expect(mockOnUpload).toHaveBeenCalledWith(mockResults);
      expect(toast.success).toHaveBeenCalledWith('CVs processed successfully');
  ...
  it('changes AI provider when select value changes', async () => {
    ...
    const fileForTest = new File(['dummy content'], 'test.pdf', { type: 'application/pdf' });
    ...
    const geminiOption = await waitFor(() => screen.getByText('Google Gemini'));
